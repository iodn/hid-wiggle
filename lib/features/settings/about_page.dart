...
class _AboutPageState extends State<AboutPage> {
  ...
  static const String _issuesUrl = 'https://github.com/iodn/hid-wiggle/issues';
  ...
  String _appName() => (_info?.appName.trim().isNotEmpty ?? false) ? _info!.appName : 'HIDWiggle';
      ...
      body: ListView(
        ...
        children: [
            ...
            child: Column(
              children: [
                ListTile(
                  ...
                  onTap: () => _launchExternal(context, _repoUrl),
                  onLongPress: () => _copy(context, _repoUrl, 'Repository link copied'),
                ),
                const Divider(height: 1),
                ListTile(
                  leading: const Icon(Icons.bug_report_outlined),
                  title: const Text('Report an issue'),
                  subtitle: const Text(_issuesUrl),
                  trailing: const Icon(Icons.open_in_new_rounded),
                  onTap: () => _launchExternal(context, _issuesUrl),
                  onLongPress: () => _copy(context, _issuesUrl, 'Issues link copied'),
                ),
                const Divider(height: 1),
                ListTile(
                  leading: const Icon(Icons.favorite_rounded),
                  title: const Text('Donate via Liberapay'),
                  subtitle: const Text(_liberapayUrl),
                  trailing: const Icon(Icons.open_in_new_rounded),
                  onTap: () => _launchExternal(context, _liberapayUrl),
                  onLongPress: () => _copy(context, _liberapayUrl, 'Liberapay link copied'),
                ),
              ],
            ),
          ),
          ...
          SectionCard(
            ...
            subtitle: 'USB HID mouse jiggler',
            leading: Icon(Icons.mouse, color: cs.primary),
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    'HIDWiggle creates a USB HID gadget on rooted Android and streams subtle human-like pointer motion to keep systems awake. It focuses on safety, reliable UDC binding, and realistic movement. No accounts, no tracking, no ads.',
                    style: theme.textTheme.bodyMedium?.copyWith(
                  ...
                  const Wrap(
                    ...
                    runSpacing: 8,
                    children: [
                      SupportPill(icon: Icons.mouse, label: 'Mouse HID'),
                      SupportPill(icon: Icons.schedule, label: 'Scheduler'),
                      SupportPill(icon: Icons.usb, label: 'ConfigFS'),
                      SupportPill(icon: Icons.security, label: 'Root required'),
                    ],
                  ),
                ],
              ),
      ...
      child = ClipRRect(
        ...
        child: Image.asset(
          ...
          errorBuilder: (context, error, stackTrace) {
            return Icon(Icons.mouse, size: size * 0.6, color: iconColor);
          },
        ),
      );
    } else {
      child = Icon(Icons.mouse, size: size * 0.6, color: iconColor);
    }
